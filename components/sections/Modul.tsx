  ...
  return (
    ...
    >
      <div className="bg-biru-gelap relative rounded-[20px] px-3 sm:px-6 py-8 mx-2 sm:mx-6 shadow-md z-30">
        {/* Title */}
        <div className="absolute -top-5 left-1/2 -translate-x-1/2 bg-yellow-500 text-white px-6 sm:px-16 py-2 rounded-full shadow-md text-base sm:text-lg font-bold z-20">
          Modul Terupdate
        </div>
          ...
          <Slider {...sliderSettings}>
            {modulData.map((modul, index) => (
              <div key={index} className="p-3 text-center">
                <img
                  src={`${CDN_ASSET_URL}/${modul.image}`} // 🔥 FIX: sebelumnya salah
                  alt={modul.title}
                  loading="lazy"
