...
export default function Features() {
  ...
  w-full h-auto"
    >
      <div className="bg-white relative rounded-[20px] px-3 sm:px-6 py-8 mx-2 sm:mx-6 shadow-md mt-[8rem]">
        {/* Title */}
        <div
          ...
          <div className="hidden md:block">
            <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-5">
              {programs.map((program, index) => (
                ...
                >
                  {program.tag && (
                    <div className="absolute top-0 left-0 w-full bg-red-700 text-white py-2 text-sm rounded-t-lg font-semibold text-center">
                      {program.tag}
                    </div>
                  )}

                  <h4
                    className={`text-xl font-bold text-red-800 mb-1 ${program.title === "Gold" ? "mt-10" : ""}`}
                  >
                    {program.title}
                  </h4>
            ...
            <Slider {...sliderSettings}>
              {programs.map((program, index) => (
                <div key={index} className="p-2">
                  <div
                    className={`bg-white p-5 shadow-md rounded-xl ${
                      program.title === "Gold"
                  ...
                  >
                    {program.tag && (
                      <div className="absolute top-0 left-0 w-full bg-red-700 text-white py-2 text-xs font-semibold text-center">
                        {program.tag}
                      </div>
                    )}

                    <h4
                      className={`text-lg font-bold text-red-800 mb-1 ${program.title === "Gold" ? "mt-10" : ""}`}
                    >
                      {program.title}
                    </h4>
                    ...
                    <div className="text-xs text-yellow-600 font-medium mb-4 whitespace-pre-line">
                      {program.description}
                    </div>
        ...
        <div className="text-left px-4 sm:px-8 pt-6">
          ...
          <ul className="list-disc list-inside ml-4 text-xs sm:text-sm text-gray-400 space-y-0.5">
